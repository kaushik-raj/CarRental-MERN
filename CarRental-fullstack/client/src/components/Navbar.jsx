...
const Navbar = () => {
    ...
    // useAppContext is a custom hook that returns the context values
    // It provides access to the app context values like user, logout, isOwner, axios
    // setshowLogin is used to toggle the login modal
    // setIsOwner is used to set the owner status
    const {setShowLogin, user, logout, isOwner, axios, setIsOwner} = useAppContext()


    // location is used to find the current path
    const location = useLocation()
    // open is used to toggle the mobile menu
    const [open, setOpen] = useState(false)
    // navigate is used to navigate to different pages
    const navigate = useNavigate()

    // Change the user role to owner
    // This function is called when the user clicks on the "List cars" button
